...
const { cleanSpaces,
        asSnakeCase,
        cleanAccents }          = require( 'easy/lib/string' )

module.exports.command = 'generate [bundle]'
module.exports.describe = 'Allows to create bundle with console support'
module.exports.handler = argv => {
    application.start()

    // const argv                  = minimist( process.argv.slice( 2 ) )
    const schemaDatabaseService = application.container.getService( 'database.schema' )
    ...
    if ( nameBundleFromArgv.length > 0 ) {
        ...
        const skeletonBundlePath    = path.resolve( `${kernel.path.root}/node_modules/easy/.cache/bundles/skeleton` )
