...
const { upperFirst, lowerFirst, camelCase } = require( 'lodash' )
...
class Bundle {
    ...
    constructor( name, kernel ) {
        ...
        this.formattedNameCapitalized = upperFirst( camelCase( name ) )
        this.formattedNameDecapitalized = lowerFirst( camelCase( name ) )
        this.skeleton = null
    }
    ...
    async walkerOnDirectory( root, dirStatsArray, next, reject ) {
        ...
        await Promise.all( dirStatsArray.map( async directoryStat => {
            const newDirectoryName = directoryStat.name.replace( /Skeleton/gi, this.name )
    ...
    async walkerOnFile( root, fileStats, next, reject ) {
        ...
        const newPath = path.resolve( `${this.path}/${skeletonFilePath.replace( this.skeleton.path, '' ).replace( /skeleton/g, this.name )}` )
