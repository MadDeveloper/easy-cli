...
class Bundle {
    ...
    walkerOnFile( root, fileStats, next, reject ) {
        ...
        const newFilePath = path.resolve( `${this.path}/${skeletonFilePath.replace( this.skeleton.path, '' ).replace( /Skeleton/g, this.formattedNameCapitalized ).replace( /skeleton/g, this.formattedNameDecapitalized )}` )
        ...
        if ( 'skeleton.js' !== filename.toLowerCase() && 'skeleton.repository.js' !== filename.toLowerCase() ) {
            this.createFile( newFilePath, fileContent )
                .then( next )
                .catch( reject )
        } else {
            next()
     ...
     * @returns {Promise}
     */
    createFile( newFilePath, fileContent ) {
        return new Promise( ( resolve, reject ) => {
            fs.writeFile( newFilePath, fileContent.replace( /Skeleton/g, this.formattedNameCapitalized ).replace( /skeleton/g, this.formattedNameDecapitalized ), { encoding: 'utf8' }, error => {
                if ( error ) {
                    reject( error )
                    return
                }

                fs.chmod( newFilePath, parseInt( 755, 8 ), error => {
                    if ( error ) {
                        reject( error )
                        return
                    }
